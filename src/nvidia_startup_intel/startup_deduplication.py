...
def _find_duplicate_index(
    ...
    for index, existing in enumerate(existing_candidates):
        ...
        if candidate_domain != UNKNOWN and existing_domain != UNKNOWN and candidate_domain == existing_domain:
            return index
...
def _merge_candidates(existing: "CandidateStartup", new: "CandidateStartup") -> "CandidateStartup":
    ...
    primary_url = _preferred_url(existing.primary_url, new.primary_url)
    confidence_score = round(max(existing.confidence_score, new.confidence_score) + min(len(evidences) - 1, 2) * 0.05, 2)
...
def _preferred_url(existing_url: str, new_url: str) -> str:
